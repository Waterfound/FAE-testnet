...
Source revision: `448c6c83bae0edb1cadf4e8957d9487c2d174783`
...
Before integration, `main` advanced by five commits to `448c6c83bae0edb1cadf4e8957d9487c2d174783`. The intervening changes add the PQ-08 shadow-only differential workflow and lab artifacts. The workflow declares only `contents: read`, contains no direct `secrets.*` reference and uses no `pull_request_target`. PSR-12/14 was rebound and must replay both canonical verification and CodeQL on this source revision.


## Rebinding note — revision 3

`main` advanced again to `448c6c83bae0edb1cadf4e8957d9487c2d174783` with PQ-09 shadow-only resource-economics work. The new `pq-lab-wave-g-pq09.yml` workflow declares only `contents: read`, has no direct `secrets.*` reference and uses no `pull_request_target`. This candidate must replay canonical verification and CodeQL again; prior successful runs on older bindings are evidence history only.


## Rebinding note — revision 4

`main` advanced to `448c6c83bae0edb1cadf4e8957d9487c2d174783` with MTS-11 real-browser validation and PQ Wave G/H bookkeeping. No new workflow was added. The modified Mining Tip Sync workflow remains `contents: read`, with no direct `secrets.*` reference and no `pull_request_target`. Canonical #518 and CodeQL #2 succeeded on the prior binding but are preserved only as history; this revision must replay both.
