...
async function body(req){let s='';for await(const c of req){s+=c;if(s.length>1_000_000)throw Error('body_too_large');}return s?JSON.parse(s):{};}
...
function reset(){blocks=new Map([[GENESIS,{hash:GENESIS,height:0,parent:null,origin:'genesis',createdAtMs:0,seq:0}]]);children=new Map();events=new Map();tipHash=GENESIS;reorgCount=0;seq=0;}
reset();
function blockByHash(h){return blocks.get(h)||null;}
...
const server=http.createServer(async(req,res)=>{try{
  ...
  if(u.pathname==='/control/mine'&&req.method==='POST'){if(!auth(req))return send(res,403,{ok:false,error:'forbidden'});const x=await body(req);const parent=x.parent||tipHash;const p=blocks.get(parent);if(!p)return send(res,409,{ok:false,error:'unknown_parent'});const payload={lab:'FAE_WAN_LAB_V1',height:p.height+1,parent,createdAtMs:Date.now(),origin:NODE_ID,seq:++seq,nonce:String(x.nonce??Math.floor(Math.random()*1e9))};const b={...payload,hash:hashBlockPayload(payload)};ingest(b,'local',Date.now());relay(b).catch(()=>{});return send(res,200,{ok:true,block:b});}
...
server.listen(PORT,'0.0.0.0',()=>console.log(JSON.stringify({event:'listen',nodeId:NODE_ID,region:REGION,port:PORT,peers:PEERS})));
