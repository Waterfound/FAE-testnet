...
let report={ok:true,status:'starting',format:'FAE_STABILITY_SOAK_V1',startedAt:new Date().toISOString(),targetSeconds:TARGET_MS/1000,daaHalfLifeMinutes:HALF_LIFE_MS/60000,durationHours:DURATION_MS/3600000,nodes:NODES};
...
async function snapshots(){return await Promise.all(NODES.map(n=>req(n.url,'/snapshot')))}
...
async function waitConverged(timeout=45000){const start=Date.now();let last;while(Date.now()-start<timeout){last=await statuses();if(converged(last))return last;await syncAll();await sleep(800)}throw Error('convergence_timeout:'+JSON.stringify(last))}
...
async function run(){
  ...
  report={...report,status:'running',anchorTimestamp,anchorHeight,phase:'baseline'};
  ...
  const pass=converged(final)&&pm.p95Ms<5000&&rate<0.01&&upper<0.01;
  report={...report,ok:pass,status:pass?'PASS':'FAIL',finishedAt:new Date().toISOString(),final:final.map((x,i)=>({id:NODES[i].id,height:x.height,tipHash:x.tipHash,staleBlocks:x.staleBlocks,reorgCount:x.reorgCount,produced:x.autoProduce?.produced||0})),propagation:pm,naturalStale:{staleBlocks:stale,canonicalHeight:height,totalDiscovered:n,observedRate:rate,wilsonUpper95:upper},liveness:{lossRecoveryToUnder2xHours:lossRecoveryStart&&lossRecoveryUnder2x?(lossRecoveryUnder2x-lossRecoveryStart)/3600000:null,surgeMinExpectedSec:Number.isFinite(surgeMinExpectedMs)?surgeMinExpectedMs/1000:null,maxCanonicalGapSec:maxCanonicalGapMs/1000},notes:['Independent stochastic block clocks run inside each regional node, producing natural timing races over the real WAN path.','The 30-minute DAA remains shadow-controlled by the soak coordinator; these lab blocks are not active FAE consensus blocks and do not perform production PoW.','Pass requires convergence, P95 propagation under 5 s, observed stale rate under 1%, and one-sided Wilson 95% upper bound under 1%.']};
...
http.createServer((req,res)=>{res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});res.end(JSON.stringify(req.url==='/result'?report:{ok:true,status:report.status,checkpoint:report.checkpoint||null,shadow:report.shadow||null}))}).listen(PORT,'0.0.0.0',()=>console.log(`FAE stability soak listening ${PORT}`));
