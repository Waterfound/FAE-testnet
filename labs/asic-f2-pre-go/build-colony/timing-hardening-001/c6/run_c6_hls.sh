...
EXPECTED_BLOB="d6631dbd952d6d75c302c4ce6cf6a751670ec44f"
WORK="/opt/fae-c6-hls"
LOG="/var/log/fae-c6-hls"
mkdir -p "$WORK" "$LOG"
...
PREFIX="evidence/$IID/build-colony-hls-c6"
...
# C6-specific structural gate: rotr64 must be fully inlined.
if find fae_dp6_hls_prj/sol_vu47p/syn/report -type f -name 'rotr64*_csynth.rpt' | grep -q .; then
  echo "C6_ROTR64_STANDALONE_REPORT_PRESENT" > "$LOG/c6_inline_gate.txt"
  exit 41
fi
if grep -R -q 'grp_rotr64' fae_dp6_hls_prj/sol_vu47p/.autopilot/db; then
  echo "C6_ROTR64_GROUP_STILL_PRESENT" > "$LOG/c6_inline_gate.txt"
  exit 42
fi
echo "C6_ROTR64_INLINE_CONFIRMED" > "$LOG/c6_inline_gate.txt"
