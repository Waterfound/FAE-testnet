...
export function createFullTargetShadowPeerNode({host='127.0.0.1',port=0,dataFile=null,identityFile=null,policy,trustedPrefix,initialChain=null}={}){
  ...
  async function syncPeer(peer){
    ...
    const session=await sessionFor(hello);try{
      ...
      await serial(async()=>{const currentWork=chainWork(state.chain,policy);const candidateWork=chainWork(result.candidate_chain,policy);if(candidateWork<currentWork)throw new Error('shadow_local_advanced');state={format:FULL_TARGET_SHADOW_STATE_FORMAT,chain:jsonClone(result.candidate_chain)};await persist()});
