import {PeerDiversityPolicy,normalizePeerEndpoint,networkGroupForEndpoint} from '../authoritative/peer-diversity.mjs';
...
/**
 * Research-only candidate for the discovery layer that precedes authoritative
 * peer authentication. It deliberately has no consensus or chain authority.
 *
 * Goals:
 * - a signed-descriptor flood must not evict a configured out-of-band anchor;
 * - probe ordering must prefer anchors and spread probes across network groups;
 * - failed anchors must make the readiness gate fail closed rather than vanish;
 * - memory and endpoint fanout remain bounded under identity churn.
 */
export class EclipseResistantDiscovery{
  constructor({
  ...
  evictionRank(row){
    // Lower rank is safer to evict. Out-of-band anchors are the last class that
    // may be evicted, and only when the configured bound is itself impossible.
    if(row.pinned)return 3;
  ...
  probeBatch({limit=this.maxProbeBatch}={}){
    ...
    const eligible=[...this.records.values()].filter(row=>row.nextProbeAt<=now);
    const byIdentity=new Map();
    for(const row of eligible){
      const previous=byIdentity.get(row.identityId);
      if(!previous||Number(row.pinned)>Number(previous.pinned)||row.lastProbeAt<previous.lastProbeAt||(row.lastProbeAt===previous.lastProbeAt&&row.endpoint<previous.endpoint))byIdentity.set(row.identityId,row);
    }
    ...
    const groupNames=[...groups.keys()].sort((a,b)=>{
      const ap=groups.get(a).some(row=>row.pinned),bp=groups.get(b).some(row=>row.pinned);
      if(ap!==bp)return ap?-1:1;
      const aOld=Math.min(...groups.get(a).map(row=>row.lastProbeAt)),bOld=Math.min(...groups.get(b).map(row=>row.lastProbeAt));
      return aOld-bOld||a.localeCompare(b);
    });
  ...
  markProbeResult({identityId,endpoint,ok}){
    const id=asIdentity(identityId),normalized=normalizePeerEndpoint(endpoint),row=this.records.get(recordKey(id,normalized));
    if(!row)throw new Error('Unknown discovery candidate');
    ...
    row.authenticated=false;row.failures=Math.min(row.failures+1,32);
    const backoff=Math.min(this.maxFailureBackoffMs,this.failureBackoffMs*(2**Math.min(row.failures-1,16)));row.nextProbeAt=now+backoff;
    return{authenticated:false,failures:row.failures,nextProbeAt:row.nextProbeAt};
  }

  disconnect(identityId,endpoint){
    const id=asIdentity(identityId),normalized=normalizePeerEndpoint(endpoint),row=this.records.get(recordKey(id,normalized));
    if(!row)return false;row.authenticated=false;row.nextProbeAt=Math.min(row.nextProbeAt,this.now());return true;
  }
  ...
  assessment(){
    const diversity=this.diversity.evaluate(this.authenticatedObservations()),knownPinned=[...this.records.values()].filter(row=>row.pinned).length,livePinned=new Set([...this.records.values()].filter(row=>row.pinned&&row.authenticated).map(row=>row.identityId)).size;
    return{ready:diversity.ready,state:diversity.ready?'READY':'HOLD',knownRecords:this.records.size,knownPinned,livePinned,...diversity};
  }
