...
const NODE_CAPABILITIES=['headers-first','common-ancestor-v1','signed-peer-framing-v1','encrypted-peer-channel-v1','cumulative-work','reorg-mempool-recovery','recovered-tx-repropagation','coordinator-gossip-v1','peer-descriptor-gossip-v1','peer-diversity-selection-v1'];
...
export function createAuthoritativeV4PeerNode({
  ...
  async function syncPeer(peer,{source='configured',expectedIdentityId=null}={}){
    ...
    try{
      ...
      if(BigInt(hello.chain_work)<=chainWork(state.chain))return{adopted:false,reason:'not_more_work',peer_id:hello.peer_id};
      ...
      if(ancestorFrame.request_nonce!==request_nonce)throw new Error('peer_ancestor_nonce_mismatch');const ancestor=ancestorFrame.ancestor;if(!ancestor||!Number.isSafeInteger(Number(ancestor.height))||Number(ancestor.height)<0||Number(ancestor.height)>(tip(state)?.height??0))throw new Error('peer_invalid_ancestor');const localAncestor=blockAtHeight(state,Number(ancestor.height));if(!localAncestor||localAncestor.hash!==ancestor.hash)throw new Error('peer_ancestor_not_local');if(Number(hello.height)<=Number(ancestor.height))return{adopted:false,reason:'no_branch',peer_id:hello.peer_id};

      const from=Number(ancestor.height)+1,target=Number(hello.height),prefix=state.chain.slice(0,Number(ancestor.height)),headersPage=await fetchPeerHeaders(session,hello.peer_id,from,target);validateHeaderSequence(prefix,headersPage,{activationHeight});if(Number(ancestor.height)===0&&headersPage[0]?.hash!==CANONICAL_GENESIS_HASH)throw new Error('peer_genesis_checkpoint_mismatch');if(headersPage.at(-1)?.hash!==hello.tip_hash)throw new Error('peer_tip_changed_during_headers');
  ...
  async function syncPeers(){if(syncRunning)return[];syncRunning=true;try{refreshSelfDescriptor();const results=[],attempted=new Set();for(const peer of configuredPeers){attempted.add(peer);try{results.push({peer,...await syncPeer(peer,{source:'configured'})})}catch(error){results.push({peer,adopted:false,error:error.message})}}await authenticateDiscoveredCandidates();const selected=diversity.select(authenticatedObservations(),{limit:MAX_SYNC_SELECTION});for(const record of selected){if(attempted.has(record.endpoint))continue;attempted.add(record.endpoint);try{results.push({peer:record.endpoint,...await syncPeer(record.endpoint,{source:record.source||'selected',expectedIdentityId:record.identityId})})}catch(error){results.push({peer:record.endpoint,adopted:false,error:error.message})}}return results}finally{syncRunning=false}}
