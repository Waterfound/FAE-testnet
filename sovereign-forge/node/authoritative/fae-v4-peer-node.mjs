...
export function createAuthoritativeV4PeerNode({
  ...
  async function syncPeer(peer){
    ...
    try{
      ...
      const previous=state,reconciled=reconcileDetachedTransactions(candidate,previous);state=reconciled.state;await persist();return{adopted:true,height:tip(state)?.height??0,tip_hash:tip(state)?.hash||ZERO_HASH,reaccepted:reconciled.reacceptedRecords.length,dropped:reconciled.rejected,headers_validated:headersPage.length,blocks_downloaded:downloaded.blocks.length,peer_id:hello.peer_id};
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(req.method==='POST'&&url.pathname==='/submit-block'){const body=await readJson(req);let accepted;await serial(async()=>{state=appendBlockFromSubmission(state,{header:body.header,nonce:Number(body.nonce),hash:String(body.hash||''),txids:(body.txids||[]).map(String),coinbase_outputs:body.coinbase_outputs??null},{activationHeight});accepted=tip(state);await persist()});return send(res,200,{ok:true,height:accepted.height,hash:accepted.hash,tx_count:accepted.txids.length})}
      if(req.method==='POST'&&url.pathname==='/submit-tx'){const body=await readJson(req);let accepted;await serial(async()=>{accepted=acceptTxInto(state,body.tx||body);await persist()});return send(res,200,{ok:true,txid:accepted.txid,fee_atoms:accepted.fee_atoms})}
