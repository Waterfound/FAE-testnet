...
export function createAuthoritativeV4PeerNode({
  ...
  function status(){const last=tip(state),amount=issued(state);return{ok:true,node_version:AUTHORITATIVE_NODE_VERSION,protocol_version:AUTHORITATIVE_P2P_PROTOCOL_VERSION,network:NETWORK,genesis_hash:CANONICAL_GENESIS_HASH,node_identity:identity.id,height:last?Number(last.height):0,tip_hash:last?.hash||ZERO_HASH,difficulty_bits:nextDifficulty(state.chain),target_seconds:TARGET_SECONDS,issued_atoms:amount.toString(),issued_fae:formatAtoms(amount),max_supply_atoms:MAX_SUPPLY.toString(),halving_era_blocks:HALVING_ERA_BLOCKS,chain_work:chainWork(state.chain).toString(),mempool_size:state.mempoolOrder.filter(id=>state.transactions[id]?.status==='pending').length,configured_peers:configuredPeers.length,secure_channels:secureChannels.status(),discovered_coordinators:coordinators.list().length,pplns_coinbase_activation_height:activationHeight??null,capabilities:['headers-first','common-ancestor-v1','signed-peer-framing-v1','encrypted-peer-channel-v1','cumulative-work','reorg-mempool-recovery','recovered-tx-repropagation','coordinator-gossip-v1']}}
