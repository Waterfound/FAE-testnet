...
function handshakeTranscript({network,sessionId,challenge,clientIdentityId,serverIdentityId,clientEphemeralKey,serverEphemeralKey,expiresAt}){
  return {domain:SECURE_CHANNEL_DOMAIN,version:CHANNEL_VERSION,network,sessionId,challenge,clientIdentityId,serverIdentityId,clientEphemeralKey,serverEphemeralKey,expiresAt};
}

export class SecureChannelHub{
  constructor({identity,networkId,ttlMs=DEFAULT_TTL_MS,maxSessions=DEFAULT_MAX_SESSIONS,maxMessageBytes=DEFAULT_MAX_MESSAGE_BYTES,now=()=>Date.now()}){
    if(!identity?.id||!identity?.privateKeyPem)throw new Error('Secure channel requires a node identity');
    this.identity=identity; this.networkId=networkId; this.ttlMs=ttlMs; this.maxSessions=maxSessions; this.maxMessageBytes=maxMessageBytes; this.now=now; this.sessions=new Map();
  }
  ...
  accept(initEnvelope){
    ...
    const transcript=handshakeTranscript({network:this.networkId,sessionId,challenge:init.challenge,clientIdentityId,serverIdentityId:this.identity.id,clientEphemeralKey:init.clientEphemeralKey,serverEphemeralKey:serverEphemeral.publicKey,expiresAt});
    const keys=deriveKeys(serverEphemeral.privateKey,init.clientEphemeralKey,transcript);
    this.sessions.set(sessionId,{clientIdentityId,receiveKey:keys.c2s,sendKey:keys.s2c,nextSequence:0,expiresAt});
  ...
  openRequest(frame){
    ...
    session.nextSequence+=1; return {clientIdentityId:session.clientIdentityId,sessionId:frame.sessionId,sequence:frame.sequence,message};
  }
  ...
  status(){this.prune(); return {activeSessions:this.sessions.size,maximumSessions:this.maxSessions,ttlMs:this.ttlMs}}
}

export class SecurePeerSession{
  constructor({baseUrl,networkId,sessionId,expiresAt,sendKey,receiveKey,maxMessageBytes=DEFAULT_MAX_MESSAGE_BYTES}){
    this.baseUrl=baseUrl.replace(/\/$/,''); this.networkId=networkId; this.sessionId=sessionId; this.expiresAt=expiresAt; this.sendKey=sendKey; this.receiveKey=receiveKey; this.maxMessageBytes=maxMessageBytes; this.nextSequence=0; this.closed=false;
  }
...
export async function establishSecurePeerSession({baseUrl,identity,networkId,expectedServerId}){
  const clientEphemeral=ephemeralKeyPair(),challenge=randomBytes(32).toString('hex');
  const initEnvelope=signEnvelope(identity,'secure-channel-init',{network:networkId,expectedServerId,challenge,clientEphemeralKey:clientEphemeral.publicKey});
  ...
  return new SecurePeerSession({baseUrl,networkId,sessionId:accepted.sessionId,expiresAt:accepted.expiresAt,sendKey:keys.c2s,receiveKey:keys.s2c});
}
