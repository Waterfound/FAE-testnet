...
export class ShareCoordinator{
  ...
  async createWork(address){
    ...
    const proofPayload={proofVersion:1,coordinatorId:this.identity.id,network:this.networkId,jobId,ledgerState,pplnsWeights:weights,weightsCommitment,templateCommitment,height:Number(candidate.header.height),previousHash:String(candidate.header.previous_hash),blockDifficultyBits:blockBits,targetDifficultyBits:shareDifficultyBits,payoutCommitment,expiresAt:new Date(expiresAt).toISOString()};
    ...
    return{...candidate,jobId,coordinatorId:this.identity.id,workMode:'share',targetDifficultyBits:shareDifficultyBits,blockDifficultyBits:blockBits,payoutCommitment,ledgerState,pplnsWeights:weights,weightsCommitment,templateCommitment,expiresAt:new Date(expiresAt).toISOString(),workProof};
  }
