...
function optionsFor(dir,name,{policy=null,descriptorOverride=null}={}){
...
try{
  ...
  // Generic hello hooks may not override protocol-owned fields.
  ...
  // Backward-compatible default mode ignores unknown signed hello extensions.
  const legacyToCandidate=await legacy.syncPeer(a.baseUrl());
  assert.equal(legacyToCandidate.adopted,false);assert.equal(legacyToCandidate.reason,'already_current');
  assert.equal(legacy.status().authenticated_peers,1);
  ...
  console.log(JSON.stringify({status:'PASS',policy_id:descriptorA.policy_id,matched:true,mismatch_rejected:true,missing_rejected:true,tamper_rejected:true,default_backward_compatible:true,authentication_fail_closed:true}));
}finally{
