...
test('Authoritative protocol 6 performs encrypted headers-first common-ancestor recovery without regressing reorg mempool safety',async context=>{
  ...
  await b.start();await c.start();context.after(()=>b.close());context.after(()=>c.close());
  const a=createAuthoritativeV4PeerNode({initialState:baseState,dataFile:join(dir,'a-state.json'),identityFile:join(dir,'a-id.json'),peerTrustFile:join(dir,'a-trust.json'),peers:[b.baseUrl()]});
  await a.start();context.after(()=>a.close());
  ...
  const accepted=await submitTx(a.baseUrl(),tx),txid=hashHex(tx);assert.equal(accepted.txid,txid);assert.equal(accepted.fee_atoms,'1000');
  ...
  assert.equal(recovery.adopted,true);assert.equal(recovery.headers_validated,2);assert.equal(recovery.blocks_downloaded,2);assert.equal(recovery.reaccepted,1);assert.equal(a.status().height,14);assert.equal(a.status().tip_hash,b.status().tip_hash);assert.equal(a.status().mempool_size,1);assert.equal(findTx(a.getState(),txid)?.status,'pending');
  ...
  console.log(JSON.stringify({ok:true,protocol:6,nodes:3,height:15,headers_first:true,common_ancestor:12,recovered_txid:txid,reconfirmed_height:15,persistent_identity:true,tofu_continuity:true}));
});
