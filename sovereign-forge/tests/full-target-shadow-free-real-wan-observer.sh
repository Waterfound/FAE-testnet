...
HOSTS=$(jq -nc --argjson a "$A_META" --argjson b "$B_META" --argjson c "$C_META" '[ $a.hostname,$b.hostname,$c.hostname ]')
[[ "$(jq 'unique|length' <<<"$HOSTS")" == 3 ]]
D_HOST=$(hostname)
! jq -e --arg d "$D_HOST" 'index($d)!=null' <<<"$HOSTS" >/dev/null
...
wan_post FAE_WAN_OBSERVER_PHASE "$(jq -nc --arg run "$GITHUB_RUN_ID" --arg phase weak --arg observer_host "$D_HOST" --arg tip "$WEAK_TIP" --arg work "$WEAK_WORK" '{run_id:$run,phase:$phase,status:"PASS",observer_hostname:$observer_host,c_tip_hash:$tip,c_chain_work:$work,public_endpoint_verified:true}')"
...
PASS=$(jq -nc \
  --arg run "$GITHUB_RUN_ID" --arg phase final --arg observer_host "$D_HOST" \
  --arg a_tip "$A_TIP" --arg a_work "$A_WORK" --arg b_tip "$B_TIP" --arg b_work "$B_WORK" \
  '{run_id:$run,phase:$phase,status:"PASS",observer_hostname:$observer_host,distinct_runner_hosts:true,public_endpoint_consistency:true,weak_state_independently_observed:true,stronger_work_selected:true,secure_context_binding:true,real_public_tunnels:true,compute_provider:"github-actions",tunnel_provider:"cloudflare-quick-tunnel",multi_provider_compute_proof:false,time_equivalent_soak:false,a_tip_hash:$a_tip,a_chain_work:$a_work,b_tip_hash:$b_tip,b_chain_work:$b_work}')
