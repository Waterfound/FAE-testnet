...
function descriptor(identity,rotationSequence=0){const issuedAt=new Date().toISOString();return signEnvelope(identity,'coordinator-descriptor',{descriptorVersion:1,type:'fairyelf-share-coordinator',coordinatorId:identity.id,network:NETWORK,endpoint:BASE,mode:'pplns-direct-pay',rotationSequence,windowSize:2048,shareDifficultyDelta:6,upstreamNodeIdentity:null,issuedAt,validUntil:new Date(Date.now()+15*60_000).toISOString()})}
function makeSession(identity,rotationChain=[],rotationSequence=0){
  ...
  const context={window:null,localStorage,crypto:webcrypto,TextEncoder,Uint8Array,URL,Date,JSON,Object,Array,Number,String,Map,Set,Promise,Error,structuredClone,NETWORK,E:new TextEncoder(),stable,normalizeCoordinatorUrl,coordinatorIdentityByBase:new Map(),
    ...
    async coordinatorApi(base,path){assert.equal(base,BASE);if(path==='/descriptor'){calls.descriptor++;return{ok:true,descriptor:descriptor(identity,rotationSequence),rotation_chain:structuredClone(rotationChain)}}if(path==='/work'){calls.work++;return{ok:true,coordinatorId:identity.id,workProof:{signer:{id:identity.id,publicKey:identity.publicKeyBase64}}}}throw new Error('unexpected path '+path)}
  };
...
// The same pinned key cannot invent a higher sequence without rotating identity.
await expectTrustFailure(makeSession(A,[],1),/sequence changed without an identity rotation/i);assert.equal(pin().rotationSequence,0);
...
const rotatedB=makeSession(B,[certAB],1);assert.equal((await rotatedB.context.coordinatorApi(BASE,'/work')).coordinatorId,B.id);assert.equal(pin().coordinatorId,B.id);assert.equal(pin().publicKey,B.publicKeyBase64);assert.equal(pin().rotationSequence,1);assert.equal(pin().rotations.length,1);

// Replaying A->B cannot roll a client already on B back to A.
await expectTrustFailure(makeSession(A,[certAB],0),/no new authorization/i);assert.equal(pin().coordinatorId,B.id);
...
const rotatedC=makeSession(C,[certAB,certBC],2);assert.equal((await rotatedC.context.coordinatorApi(BASE,'/work')).coordinatorId,C.id);assert.equal(pin().coordinatorId,C.id);assert.equal(pin().rotationSequence,2);assert.equal(pin().rotations.length,2);const pinC=sharedStorage.get('fae-coordinator-trust-v1');

// A browser absent since A can verify the entire A->B->C history without trusting an intermediary source.
sharedStorage.set('fae-coordinator-trust-v1',pinA);const offlineToC=makeSession(C,[certAB,certBC],2);assert.equal((await offlineToC.context.coordinatorApi(BASE,'/work')).coordinatorId,C.id);assert.equal(pin().rotationSequence,2);

// Sequence gaps are rejected even when signed by the correct old identity.
sharedStorage.set('fae-coordinator-trust-v1',pinA);const gap=createCoordinatorRotationCertificate({oldIdentity:A,newPublicKey:B.publicKeyBase64,networkId:NETWORK,endpoint:BASE,sequence:2,reason:'invalid sequence gap attack certificate'});await expectTrustFailure(makeSession(B,[gap],2),/sequence mismatch/i);

// Reordered chains cannot manufacture continuity.
sharedStorage.set('fae-coordinator-trust-v1',pinA);await expectTrustFailure(makeSession(C,[certBC,certAB],2),/currently trusted identity|sequence mismatch/i);

// Endpoint-bound certificates cannot be replayed on the canonical endpoint.
const wrongEndpoint=createCoordinatorRotationCertificate({oldIdentity:A,newPublicKey:B.publicKeyBase64,networkId:NETWORK,endpoint:'https://other-coordinator.example',sequence:1,reason:'wrong endpoint rotation attack certificate'});sharedStorage.set('fae-coordinator-trust-v1',pinA);await expectTrustFailure(makeSession(B,[wrongEndpoint],1),/endpoint mismatch/i);
...
sharedStorage.set('fae-coordinator-trust-v1',pinC);await expectTrustFailure(makeSession(B,[certAB,certBC],1),/no new authorization/i);assert.equal(pin().coordinatorId,C.id);

console.log('FAE coordinator cryptographic A->B->C rotation, offline catch-up, exact descriptor sequence, endpoint, future-time and rollback/replay attacks passed.');
